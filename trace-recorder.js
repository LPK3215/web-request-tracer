 ...
 *
 * Features:
 *  - XHR / fetch / WebSocket full capture
 *  - Response headers recording
 *  - Regex host/path filters + request/response header filters
 *  - In-panel filter settings (no code editing needed)
 *  - HAR + JSON dual export
 *  - Click-to-request transaction tracking
 *  - Cross-page persistence via localStorage
 *
 * Usage:
 * 1) Open any website
 * 2) Paste this script into the Console and press Enter
 * 3) Click "Start" on the floating panel (bottom-right corner)
 * 4) Perform actions you want to trace
 * 5) Click "JSON" or "HAR" to export
 */
(() => {
  ...
  const filterMatch = (u, method, reqHeaders) => {
    ...
    if (Array.isArray(f.pathContains) && f.pathContains.length > 0) {
      if (!u.pathname || !f.pathContains.some((s) => u.pathname.indexOf(s) !== -1))
        return false;
    }
  ...
  window.__WRT_TRACE_EXPORT__ = function() {
    ...
    var jsonStr = JSON.stringify(
      { meta: TRACE.meta, events: TRACE.events, transactions: TRACE.transactions,
    ...
    for (var i = 0; i < TRACE.events.length; i++) {
      ...
      if (req.body && req.body.type !== "empty" && req.body.value != null) {
        postData = {
          ...
          text: typeof req.body.value === "string" ? req.body.value : JSON.stringify(req.body.value),
        };
      }
  ...
  if (RUN.enabled) {
    ...
    console.log("[wrt] installed (idle). Ready – click Start.");
  }
})();
